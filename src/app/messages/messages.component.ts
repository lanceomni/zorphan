...
export class MessagesComponent implements OnInit {
  ...
  openConversation(accountPhone: string, phoneNumber: string) {
    if (this.selectedAccountPhone.accountPhone === accountPhone) {
      ...
      this.contactSearch.setValue(phoneNumber);
    } else {
  ...
  handleNotification(type: string, data: any) {
    switch (type) {
      ...
      case 'contactEdit':
        ...
        if (this.selectedContact && (this.selectedContact.contactId === data.contact.contactId)) {
          this.selectedContact.contact.contact1 = data.concat.contact1;
          this.selectedContact.contact.contact2 = data.concat.contact2;
          this.selectedContact.contact.phoneNumber = data.concat.phoneNumber;
          this.selectedContact.contact.lastMessageBody = data.concat.lastMessageBody;
        }
        this.updateAccountPhone();
        }
      break;
  ...
  disableSend(): boolean {
    if (!this.selectedAccountPhone.canReply
      || (!this.composedMessage.attachments.length && !this.composedMessage.urls.length)) {
      if (this.userPref.signatureOn && this.outGoingMsg.message && this.outGoingMsg.message) {
        ...
        return this.outGoingMsg.message.trim() === this.userPref.signature.trim();
